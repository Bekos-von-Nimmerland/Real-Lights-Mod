...
public class BlockFLampC90 extends BlockFLampBase 
{
	public BlockFLampC90(String name, boolean isOn) 
	{
		super(name, isOn);
	}
	@Override
    ...
    public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state)
    {
        if (!worldIn.isRemote)
        {
            ...
            {
                worldIn.setBlockState(pos, BlockInit.BLOCK_FLUORESCENT_LAMP_CAGED_90.getDefaultState().withProperty(FACING, state.getValue(FACING)), 2);
            }
            else if (!this.isOn && worldIn.isBlockPowered(pos))
            {
            	worldIn.playSound((EntityPlayer) null, pos,SoundsHandler.FLUORESCENT_LAMP_TURN_ON,SoundCategory.BLOCKS, 0.2F, 1.0F);
                worldIn.setBlockState(pos, BlockInit.BLOCK_FLUORESCENT_LAMP_CAGED_ON_90.getDefaultState().withProperty(FACING, state.getValue(FACING)), 2);
            }
        }
    }
	@Override
