...
public class BlockLBulb extends BlockLBulbBase
{
	public BlockLBulb(String name, boolean isOn) {
		super(name, isOn);
	}
	@Override
    ...
    public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state)
    {
        if (!worldIn.isRemote)
        {
            ...
            {
                worldIn.setBlockState(pos, BlockInit.BLOCK_LIGHT_BULB.getDefaultState().withProperty(FACING, state.getValue(FACING)), 2);
            }
            else if (!this.isOn && worldIn.isBlockPowered(pos))
            {
            	worldIn.playSound((EntityPlayer) null, pos,SoundsHandler.BULB_LAMP_TURN_ON,SoundCategory.BLOCKS, 0.5F, 1.0F);
                worldIn.setBlockState(pos, BlockInit.BLOCK_LIGHT_BULB_ON.setLightLevel((float) worldIn.isBlockIndirectlyGettingPowered(pos)/15).getDefaultState().withProperty(FACING, state.getValue(FACING)), 2);
            }
        }
    }
	@Override
	public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos)
	{
        if (!world.isRemote)
        {
        	...
        	{
        	//this.setLightLevel((float) world.isBlockIndirectlyGettingPowered(pos)/15);
			world.playSound((EntityPlayer) null, pos,SoundsHandler.BULB_LAMP_TURN_ON,SoundCategory.BLOCKS, 0.5F, 1.0F);
			world.setBlockState(pos, BlockInit.BLOCK_LIGHT_BULB_ON.setLightLevel((float) world.isBlockIndirectlyGettingPowered(pos)/15).getDefaultState().withProperty(FACING, state.getValue(FACING)), 2);
        	}
        	else if(this.isOn&&!(world.isBlockIndirectlyGettingPowered(pos) > 0))
        	{
			world.setBlockState(pos, BlockInit.BLOCK_LIGHT_BULB.getDefaultState().withProperty(FACING, state.getValue(FACING)), 2);
        	}
        }
	}
	@Override
	public void onBlockClicked(World world, BlockPos pos, EntityPlayer entity)
	{
		if (!world.isRemote)
		{
			...
			else if(ConfigHandler.LAMP_BREAK_BY_ITEMS&&!(EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, stack) > 0)&&!stack.isEmpty())
			{
				...
				world.setBlockState(pos,BlockInit.BLOCK_LIGHT_BULB_SHATTERED.getDefaultState().withProperty(FACING, blockState.getValue(FACING)), 2);
			}
		}
	}
	@Override
    public void onEntityCollidedWithBlock(World world, BlockPos pos, IBlockState state, Entity entityIn)
    {
		if(ConfigHandler.LAMP_BREAK_BY_PROJECTILES&&!world.isRemote)
		{
			...
			{
				world.playSound((EntityPlayer) null, pos,(this.isOn?SoundsHandler.LAMP_ON_BREAK:SoundsHandler.LAMP_OFF_BREAK),SoundCategory.BLOCKS, 1.0F, 1.0F);
				world.setBlockState(pos, BlockInit.BLOCK_LIGHT_BULB_SHATTERED.getDefaultState().withProperty(FACING, state.getValue(FACING)), 2);
			}
		}
